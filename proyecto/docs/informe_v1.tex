...
\begin{itemize}
    \item Normalizacion de datasets y geocodificacion de propiedades.
    \item Calculo de metricas espaciales y construccion del proxy de satisfaccion.
    \item Entrenamiento y validacion del modelo LightGBM.
\end{itemize}
...
\begin{itemize}
    \item Incorporar variables temporales y ampliar cobertura geografica.
    \item Integrar API y visualizaciones interactivas para usuarios finales.
    \item Validacion empirica con usuarios reales y ajuste de perfiles.
\end{itemize}

% Cronograma sintetico tipo Gantt en formato tabla
\begin{table}[H]
\centering
\caption{Cronograma sintetico (Gantt simplificado)}
{\scriptsize
...
\toprule
Actividad & S4 & S5 & S6 & S7 & S8 & S9-10 \\
\midrule
Ampliacion de datos y QA & X & X & & & & \\
Refinamiento de variables e indices & & X & X & & & \\
Ajuste y validacion de modelos & & & X & X & & \\
Prototipo API y visualizaciones & & & & X & X & \\
Validacion con usuarios y reporte final & & & & & X & X \\
\bottomrule
\end{tabular}
}
}
...
Se proyecta ejecutar estas tareas en la segunda mitad del semestre, priorizando validacion con usuarios y mejoras en cobertura de datos.
