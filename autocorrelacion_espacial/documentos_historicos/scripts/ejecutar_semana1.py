...
def mostrar_resumen_final(base_path):
    ...
    if carpeta_normalizados.exists():
        ...
        print(f"\n🗺️ DATOS NORMALIZADOS ({len(archivos_norm)}):")
    ...
    print(f"   ⚠️  Usar SIEMPRE archivos de datos_normalizados/")
    print(f"   ⚠️  NO usar archivos de datos_filtrados/ para análisis")
...
def main():
    ...
    if scripts_exitosos == len(scripts_ejecutar):
        ...
        print(f"\n⚠️ Completada con algunos errores. Revisar logs.")
        return False
