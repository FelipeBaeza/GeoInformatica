...
def detectar_crs_probable(gdf, archivo_nombre):
    ...
    print(f"   ⚠️  No se pudo detectar CRS para {archivo_nombre}, asumiendo WGS84")
    return 'EPSG:4326'
...
def normalizar_archivo(archivo_origen, carpeta_destino):
    ...
    try:
        ...
        if gdf.empty:
            print(f"      ⚠️  Archivo vacío, copiando sin cambios")
        ...
        if gdf.crs.to_epsg() != 32719:
            print(f"      🗺️  Reproyectando de {gdf.crs.to_epsg()} a 32719")
